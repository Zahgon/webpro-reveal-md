...
// writePortableVerticalPreprocessor reproduces testdata/preproc.js in full,
// including the "#^" vertical branch, for hosts with no JavaScript runtime.
// Its output is byte-identical to running the fixture under Node.
func writePortableVerticalPreprocessor(t *testing.T, cwd string) string {
	t.Helper()
	script := "#!/bin/sh\n" +
		"awk 'NR>1 && /^#/ { if (index($0, \"#^\")) print \"\\n----\\n\"; else print \"\\n---\\n\" } " +
		"{ gsub(/#\\^/, \"#\"); print }'\n"
	if err := os.WriteFile(filepath.Join(cwd, "preproc-vertical.sh"), []byte(script), 0o755); err != nil {
		t.Fatal(err)
	}
	return "./preproc-vertical.sh"
}

// TestPreprocJSFixtureSeparatesVerticalSlides pins the contract of the
// original's test/preproc.js fixture, mirrored here as testdata/preproc.js.
// The fixture inserts a horizontal separator "---" before every heading after
// the first, except headings marked "#^", which instead get a vertical
// separator "----" and have the marker rewritten back to a plain "#".
// TestShouldUsePreprocessorForMarkdown only exercises the horizontal branch,
// so the vertical branch and the marker rewrite are covered here.
func TestPreprocJSFixtureSeparatesVerticalSlides(t *testing.T) {
	cwd := emptyCwd(t)
	var preprocessor string
	switch _, err := exec.LookPath("node"); {
	case err == nil:
	...
	case runtime.GOOS == "windows":
		t.Skip("no preprocessor interpreter is available on this host")
	default:
		preprocessor = writePortableVerticalPreprocessor(t, cwd)
	}

	actual := renderWith(t, cwd,
		"# Slide A\n\ncontent\n\n#^ Vertical\n\nmore\n\n# Slide B\n\ncontent",
		jsutil.ObjectOf("preprocessor", preprocessor))

	if strings.Contains(actual, "#^") {
		t.Errorf("the #^ vertical marker leaked into the rendered deck:\n%s", actual)
	}
	assertMatches(t, actual,
		`# Slide A\s+content\s+----\s+# Vertical\s+more\s+---\s+# Slide B\s+content`)
}
