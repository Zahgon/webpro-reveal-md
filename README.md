...
The migration was validated against the JavaScript original with a differential
harness that runs both implementations side by side over 69 scenarios — 6 CLI,
50 static exports, 13 servers — across 13 fixtures, comparing stdout, stderr,
exit codes, the produced file trees and every HTTP response.

That harness and the unmodified JavaScript original are migration-time
artefacts rather than part of the shipped Go program, so they live beside this
repository in the delivery bundle under `migration-tooling/` (`tools/difftest/`
and `source_javascript/`) instead of inside it. Running them needs the
original's dependencies installed once with
